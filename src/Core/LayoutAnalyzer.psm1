...
function Get-PbiLayoutAnalysis {
    ...
    if ($allVisuals.Count -eq 0) { throw 'The selected page does not contain supported visual containers.' }
    ...
    foreach ($v in $visuals) {
        ...
        for ($i=0; $i -lt $columnClusters.Count; $i++) {
            $d=[Math]::Abs($v.X-$columnClusters[$i].Center)
            if ($d -lt $bestX) { $bestX=$d; $col=$i }
        }

        $row=0; $bestY=[double]::MaxValue
        for ($i=0; $i -lt $rowClusters.Count; $i++) {
            $d=[Math]::Abs($v.Y-$rowClusters[$i].Center)
            if ($d -lt $bestY) { $bestY=$d; $row=$i }
        }

        $colSpan=1
        if ($medianWidth -gt 0) { $colSpan=[Math]::Max(1,[int][Math]::Round($v.Width/$medianWidth)) }

        $rowSpan=1
        if ($medianHeight -gt 0) { $rowSpan=[Math]::Max(1,[int][Math]::Round($v.Height/$medianHeight)) }
